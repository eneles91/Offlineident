...
w_MK_tilde = [w_H; w_S_inv; w_K];
M_Kr = [H_rH; -S_rS_inv; K_rK];
...
w_H_ident(1:end) = w_MK_tilde_ident(1:N);

w_S_inv_ident = ones(length(w_S),1);
w_S_inv_ident(1:end) = w_MK_tilde_ident(N+1:N+2*L+1);

w_K_ident = ones(length(w_K),1);
w_K_ident(1:end) = w_MK_tilde_ident(N+2*L+2:N+2*L+N+1);
